...
app.on('ready', () => {
  ...
  ipcMain.handle('openHistory', openHistory);
  ipcMain.handle('saveHistory', (event, history) => saveHistory(event, history));
  createWindow();
});
 ...
 */
async function openHistory(): Promise<History | undefined> {
  try {
 ...
 */
async function saveHistory(event: IpcMainInvokeEvent, history: History) {
  try {
    fs.writeFileSync(
      historyFilePath,
